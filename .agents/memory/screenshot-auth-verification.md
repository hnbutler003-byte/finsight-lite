...
2. For deep views with no URL routing (e.g. lesson reading view), add a temporary dev-only `useEffect` driver in the page that reads a sessionStorage flag, polls the DOM, clicks the target testid, and optionally scrolls to the element.
