...
- Learning Modules: lessons about money topics`;
  ...
  app.post("/api/teacher/classes/:id/notifications", isTeacher, async (req: any, res) => {
    try {
      ...
      // Fan out emails to opted-in students with verified emails.
      try {
  ...
  // === EMAIL: preferences, verification, certificates, deliverability ===
  ...
  async function resolveOrgIdFor(kind: string, userId: string): Promise<string | null> {
    ...
    if (kind === "student" || kind === "guardian") {
      // Derive from any enrolled class whose teacher belongs to an org.
