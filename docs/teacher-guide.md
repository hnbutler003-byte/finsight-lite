...
Students access core lessons at `/lessons`. This area contains FinSight's built-in learning modules (Budgeting Basics, Saving Smart, Investing Fundamentals, Investing Fundamentals II, the Real Life Ready module, plus regional modules such as The Sand Dollar and Real Life Ready II for Bahamian students and JAM-DEX for Jamaican students) alongside any lessons published by teachers and Org Admins. Each lesson has learning objectives, reading sections, and a quiz that earns XP.
...
The Real Life Ready module covers practical banking skills, and its lessons are territory-specific: students see the lessons written for their environment's territory. Bahamas students see opening a bank account in the Bahamas, understanding a payslip and NIB contributions, and educational investment accounts (CFAL and RF). Jamaica students see opening a bank account in Jamaica (including the TRN requirement). Trinidad and Tobago students see opening a bank account in Trinidad and Tobago (including credit unions) and Know Your Money, a lesson on checking that T&T banknotes are genuine. The avoiding scams and fraud lesson is shared and appears for every territory. Some built-in lessons include embedded visual diagrams (for example, a chequing versus savings comparison, a step-by-step account opening flow, and a payslip breakdown) that animate into view as the student scrolls through the lesson.

The JAM-DEX module (subtitle "Jamaica's Digital Dollar") appears only for Jamaican students, following the same pattern as The Sand Dollar for Bahamian students. It currently holds one lesson, Understanding JAM-DEX, covering what the digital dollar is, how a wallet works, and why adoption has been slower than planned.

The Real Life Ready II module (subtitle "Loans, Credit & Mortgages") is a Bahamas-only sequel to Real Life Ready, with four lessons: how loans actually work, understanding credit and the Bahamian credit report system, mortgages explained, and amortization (why early payments are mostly interest).
