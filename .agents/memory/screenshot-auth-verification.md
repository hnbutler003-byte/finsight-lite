...
9. CSS transitions (e.g. grid-template-rows expand/collapse) never finish under `--virtual-time-budget`: the capture shows the mid-animation (collapsed) state. Temp-gate the transition classes off on the same sessionStorage flag so the final state renders instantly.
10. A view can deterministically rasterize BLANK across many runs (DOM dump proves content is present and correct) while sibling views paint fine. This is NOT content-dependent: do not chase icons/text as the cause. Fix: touch any source file to force a Vite HMR rebuild, then recapture — the stale module graph was the culprit. Always verify via `--dump-dom` before assuming a rendering bug.
