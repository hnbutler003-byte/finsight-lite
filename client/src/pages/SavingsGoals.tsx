              ...
              <DialogContent>
                ...
                <form onSubmit={(e) => {
                  ...
                  <div>
                    ...
                    <Input id="goal-name" data-testid="input-goal-name" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} placeholder="e.g., Emergency Fund" required className="bg-white/50 backdrop-blur-sm border-white/50 rounded-xl text-gray-800 placeholder:text-gray-400 focus:ring-2 focus:ring-violet-400" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="goal-target">Target Amount</Label>
                      <Input id="goal-target" data-testid="input-goal-target" type="number" step="0.01" min="1" value={formData.targetAmount} onChange={e => setFormData(p => ({ ...p, targetAmount: e.target.value }))} required className="bg-white/50 backdrop-blur-sm border-white/50 rounded-xl text-gray-800 placeholder:text-gray-400 focus:ring-2 focus:ring-violet-400" />
                    </div>
                    <div>
                      <Label>Currency</Label>
                      <Select value={formData.currency} onValueChange={v => setFormData(p => ({ ...p, currency: v }))}>
                        <SelectTrigger data-testid="select-goal-currency" className="bg-white/50 backdrop-blur-sm border-white/50 rounded-xl text-gray-800 focus:ring-2 focus:ring-violet-400"><SelectValue /></SelectTrigger>
                        <SelectContent>
                  ...
                  <div>
                    ...
                    <Input id="goal-deadline" data-testid="input-goal-deadline" type="date" value={formData.deadline} onChange={e => setFormData(p => ({ ...p, deadline: e.target.value }))} className="bg-white/50 backdrop-blur-sm border-white/50 rounded-xl text-gray-800 placeholder:text-gray-400 focus:ring-2 focus:ring-violet-400" />
                  </div>
                      ...
                      {!isComplete && (
                        addingTo === goal.id ? (
                          <div className="flex gap-2 mt-2">
                            <Input type="number" step="0.01" min="0.01" value={addAmount} onChange={e => setAddAmount(e.target.value)} placeholder="Amount" className="flex-1 bg-white/50 backdrop-blur-sm border-white/50 rounded-xl text-gray-800 placeholder:text-gray-400 focus:ring-2 focus:ring-violet-400" data-testid={`input-add-savings-${goal.id}`} />
